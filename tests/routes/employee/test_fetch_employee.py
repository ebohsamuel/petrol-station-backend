...
@pytest.mark.asyncio
async def test_get_all_employees(client, session):
    ...
    response = await client.get("/employee/get-all-employees")
    assert response.status_code == 200
    data = response.json()
    assert data[0]["full_name"] == "employee1"


@pytest.mark.asyncio
async def test_total_employee_count(client, session):
    employee_data = emp_schema.EmployeeCreate(
        full_name="employee1",
        email="test@example.com",
        password="password123",
    ...
    response = await client.get("/employee/get-total-employee")
    assert response.status_code == 200
    data = response.json()
    assert data["total"] == 1
